...
public class FrontendEngineerDashboard extends BaseDashboard {

    private final DBconn.DatabaseConnection db = new DBconn.DatabaseConnection();

    public FrontendEngineerDashboard(String username, int roleId) { super(username, roleId); }
    ...
    protected void createSidebar() {
        ...
        sidebar.add(makeUserPanel("Frontend Engineer", ModernTheme.FRONTEND_COLOR));
        ...
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_component = createSidebarButton("Add Component", "🎨");
        btn_component.setAlignmentX(Component.LEFT_ALIGNMENT);
        btn_component.addActionListener(e -> contentLayout.show(mainContentContainer, "component"));
        sidebar.add(btn_component);
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_framework = createSidebarButton("Frameworks", "📦");
    ...
    protected void createMainContent() {
        ...
        mainContentContainer.add(build_component(), "component");
    ...
    private JPanel build_dashboard() {
        ...
        JLabel ttl = new JLabel("Frontend Engineer Dashboard"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT);
        ...
        stats.add(new StatCard("Components","87","🎨",ModernTheme.FRONTEND_COLOR));
        stats.add(new StatCard("Pages","23","🖥",ModernTheme.INFO));
        stats.add(new StatCard("Bundle Size","342KB","📦",ModernTheme.WARNING));
        stats.add(new StatCard("Lighthouse","94","⚡",ModernTheme.SUCCESS));
        stats.add(new StatCard("Accessibility","A+","♿",ModernTheme.SUCCESS));
        stats.add(new StatCard("CSS Lines","8.2K","📝",ModernTheme.CODER_COLOR));
    ...
    private JPanel build_tasks() {
        ...
        ModernButton load = new ModernButton("Load My Tasks", ModernTheme.FRONTEND_COLOR);
    ...
    private JPanel build_component() {
        JPanel p = new JPanel(new BorderLayout()); p.setBackground(ModernTheme.DARK_BG); p.setBorder(new EmptyBorder(30,30,30,30));
        JLabel ttl=new JLabel("Add UI Component"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ...
        JTextField compName = new JTextField(20); compName.setBackground(new Color(40,51,69)); compName.setForeground(ModernTheme.DARK_TEXT); compName.setCaretColor(Color.WHITE);
        JTextField compDetails = new JTextField(20); compDetails.setBackground(new Color(40,51,69)); compDetails.setForeground(ModernTheme.DARK_TEXT); compDetails.setCaretColor(Color.WHITE);

        g.gridy=0; g.gridx=0; JLabel lbl_compName=new JLabel("Component Name:"); lbl_compName.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_compName,g); g.gridx=1; form.add(compName,g);
        g.gridy=1; g.gridx=0; JLabel lbl_compDetails=new JLabel("Details:"); lbl_compDetails.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_compDetails,g); g.gridx=1; form.add(compDetails,g);
        JLabel status=new JLabel(" "); status.setForeground(ModernTheme.SUCCESS);
        g.gridy=2; g.gridx=0; g.gridwidth=2; form.add(status,g);
        ModernButton btn=new ModernButton("Save", ModernTheme.FRONTEND_COLOR); btn.setPreferredSize(new Dimension(140,40));
        g.gridy=3; form.add(btn,g);
        btn.addActionListener(e->{
            if(compName.getText().trim().isEmpty() || compDetails.getText().trim().isEmpty()){ status.setForeground(ModernTheme.ERROR); status.setText("Fill all required fields."); return; }
            try { db.Save_Frontend_Task(loggedUser.User_Username, compName+": "+compDetails,(compName.getText().trim(), compDetails.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); compName.setText(""); compDetails.setText(""); }
    ...
    private JPanel build_framework() {
        JPanel p = new JPanel(new BorderLayout()); p.setBackground(ModernTheme.DARK_BG); p.setBorder(new EmptyBorder(30,30,30,30));
        JLabel ttl=new JLabel("Add Framework / Library"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ModernCard form=new ModernCard(ModernTheme.DARK_CARD,12); form.setLayout(new GridBagLayout());
        GridBagConstraints g=new GridBagConstraints(); g.insets=new Insets(10,20,10,20); g.fill=GridBagConstraints.HORIZONTAL;
        ...
        btn.addActionListener(e->{
            ...
            try { db.Save_Frontend_Framework(loggedUser.User_Username,(fwName.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); fwName.setText(""); }
            catch(Exception ex){ status.setForeground(ModernTheme.ERROR); status.setText("Error: "+ex.getMessage()); }
        });
        p.add(form,BorderLayout.CENTER); return p;
    }
    private JPanel build_messages() {
        ...
        ModernButton sendBtn=new ModernButton("Send",ModernTheme.FRONTEND_COLOR); sendBtn.setPreferredSize(new Dimension(120,38));
        ...
        ModernButton refresh=new ModernButton("Refresh Inbox",ModernTheme.FRONTEND_COLOR); refresh.setPreferredSize(new Dimension(160,36));
