...
public class SREDashboard extends BaseDashboard {

    private final DBconn.DatabaseConnection db = new DBconn.DatabaseConnection();

    public SREDashboard(String username, int roleId) { super(username, roleId); }
    ...
    protected void createSidebar() {
        ...
        sidebar.add(makeUserPanel("Site Reliability Engineer", ModernTheme.SRE_COLOR));
        ...
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_tool = createSidebarButton("Add Tool", "🔧");
    ...
    private JPanel build_dashboard() {
        ...
        JLabel ttl = new JLabel("Site Reliability Engineer Dashboard"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT);
        ...
        stats.add(new StatCard("Uptime","99.98%","✅",ModernTheme.SRE_COLOR));
        stats.add(new StatCard("MTTR","12min","⏱",ModernTheme.SUCCESS));
        stats.add(new StatCard("Incidents","2","🚨",ModernTheme.WARNING));
        stats.add(new StatCard("Deployments","18","🚀",ModernTheme.INFO));
        stats.add(new StatCard("Alerts","7","🔔",ModernTheme.ERROR));
        stats.add(new StatCard("SLO","98.5%","📊",ModernTheme.SUCCESS));
    ...
    private JPanel build_tasks() {
        ...
        ModernButton load = new ModernButton("Load My Tasks", ModernTheme.SRE_COLOR);
    ...
    private JPanel build_tool() {
        ...
        JLabel ttl=new JLabel("Add SRE Tool"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ...
        ModernButton btn=new ModernButton("Save", ModernTheme.SRE_COLOR); btn.setPreferredSize(new Dimension(140,40));
        ...
        btn.addActionListener(e->{
            ...
            try { db.Save_SRE_Tool(loggedUser.User_Username,(toolName.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); toolName.setText(""); }
    ...
    private JPanel build_messages() {
        ...
        ModernButton sendBtn=new ModernButton("Send",ModernTheme.SRE_COLOR); sendBtn.setPreferredSize(new Dimension(120,38));
        ...
        ModernButton refresh=new ModernButton("Refresh Inbox",ModernTheme.SRE_COLOR); refresh.setPreferredSize(new Dimension(160,36));
