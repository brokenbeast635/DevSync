...
public class QAEngineerDashboard extends BaseDashboard {

    private final DBconn.DatabaseConnection db = new DBconn.DatabaseConnection();

    public QAEngineerDashboard(String username, int roleId) { super(username, roleId); }
    ...
    protected void createSidebar() {
        ...
        sidebar.add(makeUserPanel("QA Engineer", ModernTheme.QA_COLOR));
        ...
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_bugreport = createSidebarButton("Report Bug", "🐛");
        btn_bugreport.setAlignmentX(Component.LEFT_ALIGNMENT);
        btn_bugreport.addActionListener(e -> contentLayout.show(mainContentContainer, "bugreport"));
        sidebar.add(btn_bugreport);
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
    ...
    protected void createMainContent() {
        ...
        mainContentContainer.add(build_bugreport(), "bugreport");
    ...
    private JPanel build_dashboard() {
        ...
        JLabel ttl = new JLabel("QA Engineer Dashboard"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT);
        ...
        stats.add(new StatCard("Bugs Found","142","🐛",ModernTheme.QA_COLOR));
        stats.add(new StatCard("Test Cases","245","🧪",ModernTheme.INFO));
        stats.add(new StatCard("Pass Rate","92%","✅",ModernTheme.SUCCESS));
        stats.add(new StatCard("Critical","8","🚨",ModernTheme.ERROR));
        stats.add(new StatCard("Coverage","78%","📊",ModernTheme.WARNING));
        stats.add(new StatCard("Regression","12","🔄",ModernTheme.CODER_COLOR));
    ...
    private JPanel build_tasks() {
        ...
        ModernButton load = new ModernButton("Load My Tasks", ModernTheme.QA_COLOR);
    ...
    private JPanel build_bugreport() {
        JPanel p = new JPanel(new BorderLayout()); p.setBackground(ModernTheme.DARK_BG); p.setBorder(new EmptyBorder(30,30,30,30));
        JLabel ttl=new JLabel("Report a Bug"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ...
        JTextField bugProject = new JTextField(20); bugProject.setBackground(new Color(40,51,69)); bugProject.setForeground(ModernTheme.DARK_TEXT); bugProject.setCaretColor(Color.WHITE);
        JTextField severity = new JTextField(20); severity.setBackground(new Color(40,51,69)); severity.setForeground(ModernTheme.DARK_TEXT); severity.setCaretColor(Color.WHITE);
        JTextField bugDesc = new JTextField(20); bugDesc.setBackground(new Color(40,51,69)); bugDesc.setForeground(ModernTheme.DARK_TEXT); bugDesc.setCaretColor(Color.WHITE);

        g.gridy=0; g.gridx=0; JLabel lbl_bugProject=new JLabel("Project ID:"); lbl_bugProject.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_bugProject,g); g.gridx=1; form.add(bugProject,g);
        g.gridy=1; g.gridx=0; JLabel lbl_severity=new JLabel("Severity (Low/Medium/High/Critical):"); lbl_severity.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_severity,g); g.gridx=1; form.add(severity,g);
        g.gridy=2; g.gridx=0; JLabel lbl_bugDesc=new JLabel("Bug Description:"); lbl_bugDesc.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_bugDesc,g); g.gridx=1; form.add(bugDesc,g);
        JLabel status=new JLabel(" "); status.setForeground(ModernTheme.SUCCESS);
        g.gridy=3; g.gridx=0; g.gridwidth=2; form.add(status,g);
        ModernButton btn=new ModernButton("Save", ModernTheme.QA_COLOR); btn.setPreferredSize(new Dimension(140,40));
        g.gridy=4; form.add(btn,g);
        btn.addActionListener(e->{
            if(bugProject.getText().trim().isEmpty() || severity.getText().trim().isEmpty()){ status.setForeground(ModernTheme.ERROR); status.setText("Fill all required fields."); return; }
            try { db.Report_Bugs(loggedUser.User_Username, bugProject,(bugProject.getText().trim(), severity.getText().trim(), bugDesc.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); bugProject.setText(""); severity.setText(""); bugDesc.setText(""); }
    ...
    private JPanel build_messages() {
        ...
        ModernButton sendBtn=new ModernButton("Send",ModernTheme.QA_COLOR); sendBtn.setPreferredSize(new Dimension(120,38));
        ...
        ModernButton refresh=new ModernButton("Refresh Inbox",ModernTheme.QA_COLOR); refresh.setPreferredSize(new Dimension(160,36));
