...
public class MobileDeveloperDashboard extends BaseDashboard {

    private final DBconn.DatabaseConnection db = new DBconn.DatabaseConnection();

    public MobileDeveloperDashboard(String username, int roleId) { super(username, roleId); }
    ...
    protected void createSidebar() {
        ...
        sidebar.add(makeUserPanel("Mobile Developer", ModernTheme.MOBILE_COLOR));
        ...
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_platform = createSidebarButton("Add Platform", "🚀");
        btn_platform.setAlignmentX(Component.LEFT_ALIGNMENT);
        btn_platform.addActionListener(e -> contentLayout.show(mainContentContainer, "platform"));
        sidebar.add(btn_platform);
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
    ...
    protected void createMainContent() {
        ...
        mainContentContainer.add(build_platform(), "platform");
    ...
    private JPanel build_dashboard() {
        ...
        JLabel ttl = new JLabel("Mobile Developer Dashboard"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT);
        ...
        stats.add(new StatCard("App Downloads","12K","📲",ModernTheme.MOBILE_COLOR));
        stats.add(new StatCard("Crash Rate","0.3%","💥",ModernTheme.SUCCESS));
        stats.add(new StatCard("Features","28","⭐",ModernTheme.INFO));
        stats.add(new StatCard("Platforms","3","📱",ModernTheme.WARNING));
        stats.add(new StatCard("Reviews","4.7★","⭐",ModernTheme.SUCCESS));
        stats.add(new StatCard("Open Bugs","5","🐛",ModernTheme.ERROR));
    ...
    private JPanel build_tasks() {
        ...
        ModernButton load = new ModernButton("Load My Tasks", ModernTheme.MOBILE_COLOR);
    ...
    private JPanel build_platform() {
        JPanel p = new JPanel(new BorderLayout()); p.setBackground(ModernTheme.DARK_BG); p.setBorder(new EmptyBorder(30,30,30,30));
        JLabel ttl=new JLabel("Add Platform"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ...
        JTextField platName = new JTextField(20); platName.setBackground(new Color(40,51,69)); platName.setForeground(ModernTheme.DARK_TEXT); platName.setCaretColor(Color.WHITE);

        g.gridy=0; g.gridx=0; JLabel lbl_platName=new JLabel("Platform Name (iOS/Android/Flutter):"); lbl_platName.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_platName,g); g.gridx=1; form.add(platName,g);
        JLabel status=new JLabel(" "); status.setForeground(ModernTheme.SUCCESS);
        g.gridy=1; g.gridx=0; g.gridwidth=2; form.add(status,g);
        ModernButton btn=new ModernButton("Save", ModernTheme.MOBILE_COLOR); btn.setPreferredSize(new Dimension(140,40));
        g.gridy=2; form.add(btn,g);
        btn.addActionListener(e->{
            if(platName.getText().trim().isEmpty()){ status.setForeground(ModernTheme.ERROR); status.setText("Fill all required fields."); return; }
            try { db.Save_Mobile_Platform(loggedUser.User_Username,(platName.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); platName.setText(""); }
    ...
    private JPanel build_messages() {
        ...
        ModernButton sendBtn=new ModernButton("Send",ModernTheme.MOBILE_COLOR); sendBtn.setPreferredSize(new Dimension(120,38));
        ...
        ModernButton refresh=new ModernButton("Refresh Inbox",ModernTheme.MOBILE_COLOR); refresh.setPreferredSize(new Dimension(160,36));
