...
public class CoderDashboard extends BaseDashboard {

    private final DBconn.DatabaseConnection db = new DBconn.DatabaseConnection();

    public CoderDashboard(String username, int roleId) { super(username, roleId); }
    ...
    protected void createSidebar() {
        ...
        sidebar.add(makeUserPanel("Software Developer", ModernTheme.CODER_COLOR));
        ...
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_skill = createSidebarButton("My Skills", "📚");
        btn_skill.setAlignmentX(Component.LEFT_ALIGNMENT);
        btn_skill.addActionListener(e -> contentLayout.show(mainContentContainer, "skill"));
        sidebar.add(btn_skill);
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
    ...
    protected void createMainContent() {
        ...
        mainContentContainer.add(build_skill(), "skill");
    ...
    private JPanel build_dashboard() {
        ...
        JLabel ttl = new JLabel("Coder Dashboard"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT);
        ...
        stats.add(new StatCard("Tasks Done","34","✅",ModernTheme.CODER_COLOR));
        stats.add(new StatCard("Lines Written","8.4K","💻",ModernTheme.INFO));
        stats.add(new StatCard("PRs Merged","12","🔀",ModernTheme.SUCCESS));
        stats.add(new StatCard("Bugs Fixed","7","🐛",ModernTheme.WARNING));
        stats.add(new StatCard("Code Reviews","5","👁",ModernTheme.CODER_COLOR));
        stats.add(new StatCard("Skills","9","📚",ModernTheme.SUCCESS));
    ...
    private JPanel build_tasks() {
        ...
        ModernButton load = new ModernButton("Load My Tasks", ModernTheme.CODER_COLOR);
    ...
    private JPanel build_skill() {
        JPanel p = new JPanel(new BorderLayout()); p.setBackground(ModernTheme.DARK_BG); p.setBorder(new EmptyBorder(30,30,30,30));
        JLabel ttl=new JLabel("Add Technical Skill"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ...
        JTextField skillName = new JTextField(20); skillName.setBackground(new Color(40,51,69)); skillName.setForeground(ModernTheme.DARK_TEXT); skillName.setCaretColor(Color.WHITE);

        g.gridy=0; g.gridx=0; JLabel lbl_skillName=new JLabel("Skill Name:"); lbl_skillName.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_skillName,g); g.gridx=1; form.add(skillName,g);
        JLabel status=new JLabel(" "); status.setForeground(ModernTheme.SUCCESS);
        g.gridy=1; g.gridx=0; g.gridwidth=2; form.add(status,g);
        ModernButton btn=new ModernButton("Save", ModernTheme.CODER_COLOR); btn.setPreferredSize(new Dimension(140,40));
        g.gridy=2; form.add(btn,g);
        btn.addActionListener(e->{
            if(skillName.getText().trim().isEmpty()){ status.setForeground(ModernTheme.ERROR); status.setText("Fill all required fields."); return; }
            try { db.add_coder_skill((skillName.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); skillName.setText(""); }
    ...
    private JPanel build_messages() {
        ...
        ModernButton sendBtn=new ModernButton("Send",ModernTheme.CODER_COLOR); sendBtn.setPreferredSize(new Dimension(120,38));
        ...
        ModernButton refresh=new ModernButton("Refresh Inbox",ModernTheme.CODER_COLOR); refresh.setPreferredSize(new Dimension(160,36));
