...
public class SoftwareArchitectDashboard extends BaseDashboard {

    private final DBconn.DatabaseConnection db = new DBconn.DatabaseConnection();

    public SoftwareArchitectDashboard(String username, int roleId) { super(username, roleId); }
    ...
    protected void createSidebar() {
        ...
        sidebar.add(makeUserPanel("Software Architect", ModernTheme.ARCHITECT_COLOR));
        ...
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_design = createSidebarButton("Architecture", "📐");
        btn_design.setAlignmentX(Component.LEFT_ALIGNMENT);
        btn_design.addActionListener(e -> contentLayout.show(mainContentContainer, "design"));
        sidebar.add(btn_design);
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_spec = createSidebarButton("Specializations", "⭐");
        btn_spec.setAlignmentX(Component.LEFT_ALIGNMENT);
        btn_spec.addActionListener(e -> contentLayout.show(mainContentContainer, "spec"));
        sidebar.add(btn_spec);
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
    ...
    protected void createMainContent() {
        ...
        mainContentContainer.add(build_design(), "design");
        mainContentContainer.add(build_spec(), "spec");
    ...
    private JPanel build_dashboard() {
        ...
        JLabel ttl = new JLabel("Software Architect Dashboard"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT);
        ...
        stats.add(new StatCard("Systems","8","🏗",ModernTheme.ARCHITECT_COLOR));
        stats.add(new StatCard("Components","42","⚙",ModernTheme.INFO));
        stats.add(new StatCard("Design Docs","15","📐",ModernTheme.SUCCESS));
        stats.add(new StatCard("Reviews","23","👁",ModernTheme.WARNING));
        stats.add(new StatCard("Patterns","11","🔵",ModernTheme.CODER_COLOR));
        stats.add(new StatCard("Specs","6","📋",ModernTheme.SUCCESS));
    ...
    private JPanel build_tasks() {
        ...
        ModernButton load = new ModernButton("Load My Tasks", ModernTheme.ARCHITECT_COLOR);
    ...
    private JPanel build_design() {
        JPanel p = new JPanel(new BorderLayout()); p.setBackground(ModernTheme.DARK_BG); p.setBorder(new EmptyBorder(30,30,30,30));
        JLabel ttl=new JLabel("Save Architecture Design"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ...
        JTextField designNotes = new JTextField(20); designNotes.setBackground(new Color(40,51,69)); designNotes.setForeground(ModernTheme.DARK_TEXT); designNotes.setCaretColor(Color.WHITE);

        g.gridy=0; g.gridx=0; JLabel lbl_designNotes=new JLabel("Design Notes:"); lbl_designNotes.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_designNotes,g); g.gridx=1; form.add(designNotes,g);
        JLabel status=new JLabel(" "); status.setForeground(ModernTheme.SUCCESS);
        g.gridy=1; g.gridx=0; g.gridwidth=2; form.add(status,g);
        ModernButton btn=new ModernButton("Save", ModernTheme.ARCHITECT_COLOR); btn.setPreferredSize(new Dimension(140,40));
        g.gridy=2; form.add(btn,g);
        btn.addActionListener(e->{
            if(designNotes.getText().trim().isEmpty()){ status.setForeground(ModernTheme.ERROR); status.setText("Fill all required fields."); return; }
            try { db.Save_Architecture_Task(loggedUser.User_Username, "Design: "+designNotes,(designNotes.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); designNotes.setText(""); }
    ...
    private JPanel build_spec() {
        JPanel p = new JPanel(new BorderLayout()); p.setBackground(ModernTheme.DARK_BG); p.setBorder(new EmptyBorder(30,30,30,30));
        JLabel ttl=new JLabel("Add Specialization"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ModernCard form=new ModernCard(ModernTheme.DARK_CARD,12); form.setLayout(new GridBagLayout());
        GridBagConstraints g=new GridBagConstraints(); g.insets=new Insets(10,20,10,20); g.fill=GridBagConstraints.HORIZONTAL;

        JTextField specName = new JTextField(20); specName.setBackground(new Color(40,51,69)); specName.setForeground(ModernTheme.DARK_TEXT); specName.setCaretColor(Color.WHITE);

        g.gridy=0; g.gridx=0; JLabel lbl_specName=new JLabel("Specialization:"); lbl_specName.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_specName,g); g.gridx=1; form.add(specName,g);
        JLabel status=new JLabel(" "); status.setForeground(ModernTheme.SUCCESS);
        g.gridy=1; g.gridx=0; g.gridwidth=2; form.add(status,g);
        ModernButton btn=new ModernButton("Save", ModernTheme.ARCHITECT_COLOR); btn.setPreferredSize(new Dimension(140,40));
        g.gridy=2; form.add(btn,g);
        btn.addActionListener(e->{
            if(specName.getText().trim().isEmpty()){ status.setForeground(ModernTheme.ERROR); status.setText("Fill all required fields."); return; }
            try { db.Save_Specialization(loggedUser.User_Username,(specName.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); specName.setText(""); }
            catch(Exception ex){ status.setForeground(ModernTheme.ERROR); status.setText("Error: "+ex.getMessage()); }
        });
        p.add(form,BorderLayout.CENTER); return p;
    }
    private JPanel build_messages() {
        ...
        ModernButton sendBtn=new ModernButton("Send",ModernTheme.ARCHITECT_COLOR); sendBtn.setPreferredSize(new Dimension(120,38));
        ...
        ModernButton refresh=new ModernButton("Refresh Inbox",ModernTheme.ARCHITECT_COLOR); refresh.setPreferredSize(new Dimension(160,36));
