...
public class DataEngineerDashboard extends BaseDashboard {

    private final DBconn.DatabaseConnection db = new DBconn.DatabaseConnection();

    public DataEngineerDashboard(String username, int roleId) { super(username, roleId); }
    ...
    protected void createSidebar() {
        ...
        sidebar.add(makeUserPanel("Data Engineer", ModernTheme.DATA_ENGINEER_COLOR));
    ...
    private JPanel build_dashboard() {
        ...
        JLabel ttl = new JLabel("Data Engineer Dashboard"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT);
        ...
        stats.add(new StatCard("Pipelines","14","⚡",ModernTheme.DATA_ENGINEER_COLOR));
        stats.add(new StatCard("TB Processed","4.2","🗄",ModernTheme.INFO));
        stats.add(new StatCard("Jobs Today","32","🔄",ModernTheme.SUCCESS));
        stats.add(new StatCard("Failures","2","❌",ModernTheme.ERROR));
        stats.add(new StatCard("Tables","87","📋",ModernTheme.WARNING));
        stats.add(new StatCard("Latency","180ms","⏱",ModernTheme.CODER_COLOR));
    ...
    private JPanel build_tasks() {
        ...
        ModernButton load = new ModernButton("Load My Tasks", ModernTheme.DATA_ENGINEER_COLOR);
    ...
    private JPanel build_messages() {
        ...
        ModernButton sendBtn=new ModernButton("Send",ModernTheme.DATA_ENGINEER_COLOR); sendBtn.setPreferredSize(new Dimension(120,38));
        ...
        ModernButton refresh=new ModernButton("Refresh Inbox",ModernTheme.DATA_ENGINEER_COLOR); refresh.setPreferredSize(new Dimension(160,36));
