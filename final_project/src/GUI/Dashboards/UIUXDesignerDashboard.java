...
public class UIUXDesignerDashboard extends BaseDashboard {

    private final DBconn.DatabaseConnection db = new DBconn.DatabaseConnection();

    public UIUXDesignerDashboard(String username, int roleId) { super(username, roleId); }
    ...
    protected void createSidebar() {
        ...
        sidebar.add(makeUserPanel("UI/UX Designer", ModernTheme.UIUX_COLOR));
        ...
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_wireframe = createSidebarButton("Wireframes", "✏");
        btn_wireframe.setAlignmentX(Component.LEFT_ALIGNMENT);
        btn_wireframe.addActionListener(e -> contentLayout.show(mainContentContainer, "wireframe"));
        sidebar.add(btn_wireframe);
        sidebar.add(Box.createRigidArea(new Dimension(0, 6)));
        ModernButton btn_tool = createSidebarButton("Design Tools", "🛠");
    ...
    protected void createMainContent() {
        ...
        mainContentContainer.add(build_wireframe(), "wireframe");
    ...
    private JPanel build_dashboard() {
        ...
        JLabel ttl = new JLabel("UI/UX Designer Dashboard"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT);
        ...
        stats.add(new StatCard("Designs","34","🎨",ModernTheme.UIUX_COLOR));
        stats.add(new StatCard("Prototypes","12","📱",ModernTheme.INFO));
        stats.add(new StatCard("Wireframes","28","✏",ModernTheme.SUCCESS));
        stats.add(new StatCard("User Tests","9","👥",ModernTheme.WARNING));
        stats.add(new StatCard("Assets","156","🖼",ModernTheme.CODER_COLOR));
        stats.add(new StatCard("Tools","7","🛠",ModernTheme.SUCCESS));
    ...
    private JPanel build_tasks() {
        ...
        ModernButton load = new ModernButton("Load My Tasks", ModernTheme.UIUX_COLOR);
    ...
    private JPanel build_wireframe() {
        JPanel p = new JPanel(new BorderLayout()); p.setBackground(ModernTheme.DARK_BG); p.setBorder(new EmptyBorder(30,30,30,30));
        JLabel ttl=new JLabel("Add Wireframe / Design"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ...
        JTextField wfDetails = new JTextField(20); wfDetails.setBackground(new Color(40,51,69)); wfDetails.setForeground(ModernTheme.DARK_TEXT); wfDetails.setCaretColor(Color.WHITE);

        g.gridy=0; g.gridx=0; JLabel lbl_wfDetails=new JLabel("Design Details:"); lbl_wfDetails.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_wfDetails,g); g.gridx=1; form.add(wfDetails,g);
        JLabel status=new JLabel(" "); status.setForeground(ModernTheme.SUCCESS);
        g.gridy=1; g.gridx=0; g.gridwidth=2; form.add(status,g);
        ModernButton btn=new ModernButton("Save", ModernTheme.UIUX_COLOR); btn.setPreferredSize(new Dimension(140,40));
        g.gridy=2; form.add(btn,g);
        btn.addActionListener(e->{
            if(wfDetails.getText().trim().isEmpty()){ status.setForeground(ModernTheme.ERROR); status.setText("Fill all required fields."); return; }
            try { db.Save_Wireframe(loggedUser.User_Username,(wfDetails.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); wfDetails.setText(""); }
    ...
    private JPanel build_tool() {
        JPanel p = new JPanel(new BorderLayout()); p.setBackground(ModernTheme.DARK_BG); p.setBorder(new EmptyBorder(30,30,30,30));
        JLabel ttl=new JLabel("Add Design Tool"); ttl.setFont(ModernTheme.TITLE_FONT); ttl.setForeground(ModernTheme.DARK_TEXT); p.add(ttl,BorderLayout.NORTH);
        ModernCard form=new ModernCard(ModernTheme.DARK_CARD,12); form.setLayout(new GridBagLayout());
        GridBagConstraints g=new GridBagConstraints(); g.insets=new Insets(10,20,10,20); g.fill=GridBagConstraints.HORIZONTAL;
        ...
        g.gridy=0; g.gridx=0; JLabel lbl_toolName=new JLabel("Tool Name (Figma/XD/Sketch...):"); lbl_toolName.setForeground(ModernTheme.DARK_TEXT_SECONDARY); form.add(lbl_toolName,g); g.gridx=1; form.add(toolName,g);
        JLabel status=new JLabel(" "); status.setForeground(ModernTheme.SUCCESS);
        g.gridy=1; g.gridx=0; g.gridwidth=2; form.add(status,g);
        ModernButton btn=new ModernButton("Save", ModernTheme.UIUX_COLOR); btn.setPreferredSize(new Dimension(140,40));
        g.gridy=2; form.add(btn,g);
        btn.addActionListener(e->{
            ...
            try { db.Save_Design_Tool(loggedUser.User_Username,(toolName.getText().trim()); status.setForeground(ModernTheme.SUCCESS); status.setText("Saved!"); toolName.setText(""); }
            catch(Exception ex){ status.setForeground(ModernTheme.ERROR); status.setText("Error: "+ex.getMessage()); }
        });
        p.add(form,BorderLayout.CENTER); return p;
    }
    private JPanel build_messages() {
        ...
        ModernButton sendBtn=new ModernButton("Send",ModernTheme.UIUX_COLOR); sendBtn.setPreferredSize(new Dimension(120,38));
        ...
        ModernButton refresh=new ModernButton("Refresh Inbox",ModernTheme.UIUX_COLOR); refresh.setPreferredSize(new Dimension(160,36));
